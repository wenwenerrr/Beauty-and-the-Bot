...
### Whole pipeline
Here we build the recommendation pipeline from the initial user query input to the chatbot answer, which is shown in the 'Language_model_final.ipynb'. To run it, you should put 'Final_Project.py' 'recommend_api.py' 'Language_model_final.ipynb' 'client.ipynb' in the same directory, change CATALOG_JSONL, OUT_DIR in 'Final_Project.py' to your own route, then enter the environment and type 'python recommend_api.py' in the terminal to start the recommendation system api. Then you should follow the instruction of how to load the chatbot model and fill in the 'Language_model_final.ipynb' code. Then you can run the 'Language_model_final.ipynb' code.
